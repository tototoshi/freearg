        ...
        case Some(s) =>
          try { ME.pure(s.toInt) }
            ...
            case e: NumberFormatException =>
              ME.raiseError(new ArgParserException(s"${name} could not be parsed as int"))
          }
        case None =>
          defaultValue.map(ME.pure).getOrElse(ME.raiseError(new ArgParserException(s"option is not specified: $name")))
